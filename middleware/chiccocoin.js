...
class Chiccocoin {
  ...
  mine (req, res, next) {
    ...
    const lastProof = lastBlock.proof  // 未创建前最新块的工作量
    ...
    this.blockchain.newTransaction('0', process.env.NODE_NAME, 1) //初始化新块的交易记录

    // Forge the new Block by adding it to the chain
    const previousHash = this.blockchain.hash(lastProof) // 计算上一个块的哈希值
    const newBlock = this.blockchain.newBlock(proof, previousHash) //新建块
